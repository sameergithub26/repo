...
for codec_name, info in selected.items():
    ...
    if not os.path.exists(path):
        print(f"  ⚠️  {codec_name}: model not found")
        continue
...
def extract_token_features(tokens_np):
    """Extract statistical features from tokens (8, 750)"""
    # Codebook activity
    low_activity = tokens_np[:4].std()   # coarse codebooks
    high_activity = tokens_np[4:].std()  # fine codebooks
...
def compute_confidence_weights(logits_dict, base_weights):
    """
    Boost models that are more confident
    logits_dict: {model_name: logits tensor (1, 2)}
    """
    adjusted = {}
    for name, logits in logits_dict.items():
        probs = torch.softmax(logits, dim=1)[0]
        confidence = abs(probs[1] - 0.5) * 2  # 0 to 1
        
        # Boost confident models
...
for _, row in tqdm(df.iterrows(), total=len(df), desc="Ultimate Ensemble"):
    ...
    try:
        ...
        # Step 4: Apply confidence weighting on top of sample-adaptive
